...
if (!parentPort) {
  throw new Error('whisper.worker must be run as a worker thread');
}
...
port.on('message', async (msg: WorkerRequest) => {
  ...
  if (msg.type === 'transcribe') {
    try {
      // Dynamic import of whisper.cpp bindings
      // eslint-disable-next-line @typescript-eslint/no-var-requires
      const whisper = require('whisper.cpp');

      const modelPath = msg.config.modelPath ?? '';
      const language = msg.config.language ?? 'en';

      const result = await whisper.transcribe(msg.wavPath, {
        modelPath,
        language,
      });
