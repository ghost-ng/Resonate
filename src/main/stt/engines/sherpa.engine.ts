...
export class SherpaEngine implements SttEngine {
  ...
  async transcribe(
    ...
    if (!available) {
      throw new Error('sherpa-onnx is not installed or not available');
    }
    ...
    return new Promise<TranscriptSegment[]>((resolve, reject) => {
      ...
      const timeout = setTimeout(() => {
        reject(new Error('Sherpa worker timed out'));
        this.disposeWorker();
  ...
  private getWorker(): Worker {
    if (!this.worker) {
      const workerPath = path.join(__dirname, '..', 'workers', 'sherpa.worker.js');
